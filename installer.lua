-- McOS 1.0 installer for CC:Tweaked
-- Installs or repairs McOS while preserving user data and settings.
...
local function writeFile(path, data)
    ...
    local temp = path .. ".mcos_new"
    if fs.exists(temp) then pcall(fs.delete, temp) end
    local h = fs.open(temp, "w")
    if not h then return false, "Unable to open temporary file for " .. path end
    local ok, err = pcall(h.write, data)
    pcall(h.close)
    if not ok then pcall(fs.delete, temp) return false, tostring(err) end
    local check = readFile(temp)
    if check ~= data then pcall(fs.delete, temp) return false, "Write verification failed for " .. path end
    if fs.exists(path) then
        local deleted, deleteErr = pcall(fs.delete, path)
        if not deleted then pcall(fs.delete, temp) return false, tostring(deleteErr) end
    end
    local moved, moveErr = pcall(fs.move, temp, path)
    if not moved then pcall(fs.delete, temp) return false, tostring(moveErr) end
    return true
end
...
local function uniqueDir(base)
    local path, n = base, 1
    while fs.exists(path) do path = base .. "_" .. n n = n + 1 end
    return path
end

local payload = {
    { path = '/mcos/system/main.lua', data = [===[-- McOS 1.0 for CC:Tweaked
...
if not ok then
    ...
    { path = '/mcos/apps/README.txt', data = [===[McOS 1.0 external apps
======================
...
External apps are ordinary Lua code and are not sandboxed. Only install code you trust.
    ...
    { path = '/mcos/apps/hello.lua', data = [===[-- Example McOS 1.0 external app.
local api = ...
...
return {
    ...
    { path = '/startup.lua', data = [===[-- McOS 1.0 bootstrap
...
term.clear()
term.setCursorPos(1, 1)
print("McOS 1.0 installer " .. VERSION)
print(string.rep("-", 27))

local requiredDirs = { "/mcos", "/mcos/system", "/mcos/apps", "/mcos/data", "/mcos/user", "/mcos/user/notes", "/mcos/user/music", "/mcos/user/inbox", "/mcos/trash", "/mcos/backups" }
for _, path in ipairs(requiredDirs) do
    if not ensureDir(path) then error("Unable to create " .. path, 0) end
end

local totalBytes = 0
for _, file in ipairs(payload) do totalBytes = totalBytes + #file.data end
local free = fs.getFreeSpace("/")
if type(free) == "number" and free < totalBytes + 16384 then
    error("Not enough free space. Need about " .. math.ceil((totalBytes + 16384) / 1024) .. " KB.", 0)
end

-- Compile all Lua payloads before changing any installed file.
for _, file in ipairs(payload) do
    if file.path:lower():match("%.lua$") then
        local fn, err = load(file.data, "@" .. file.path, "t", _ENV)
        if not fn then error("Installer payload has a Lua syntax error in " .. file.path .. ": " .. tostring(err), 0) end
    end
end

local backupDir = uniqueDir("/mcos/backups/" .. timestamp() .. "_before_1.0")
ensureDir(backupDir)
local originals = {}

local function backupTarget(path)
    path = canonical(path)
    originals[path] = { existed = fs.exists(path), backup = nil }
    if originals[path].existed then
        local name = path:gsub("^/", ""):gsub("/", "__")
        local target = fs.combine(backupDir, name)
        local ok, err = pcall(fs.copy, path, target)
        if not ok then error("Unable to back up " .. path .. ": " .. tostring(err), 0) end
        originals[path].backup = target
    end
end

for _, file in ipairs(payload) do
    if not (file.optional and fs.exists(file.path)) then backupTarget(file.path) end
end

if fs.exists("/.mcos") and not fs.exists("/mcos/legacy_mcos1") then
    local ok, err = pcall(fs.copy, "/.mcos", "/mcos/legacy_mcos1")
    if ok then print("Preserved McOS 1.x data") else print("Warning: could not copy legacy data: " .. tostring(err)) end
end

local installed = {}
local ok, installErr = pcall(function()
    -- startup.lua is last in payload, so a failed core install cannot replace the boot file first.
    for _, file in ipairs(payload) do
        if file.optional and fs.exists(file.path) then
            print("Preserved " .. file.path)
        else
            -- Mark the target as changed before replacing it. If writeFile deletes
            -- the old file and then fails, rollback must still restore that target.
            installed[#installed + 1] = file.path
            local success, err = writeFile(file.path, file.data)
...
if not ok then
    term.setTextColor(colors.red)
    print("\nInstallation failed: " .. tostring(installErr))
    print("Rolling back changed files...")
    for i = #installed, 1, -1 do
        local path = installed[i]
        local original = originals[path]
        pcall(fs.delete, path)
        if original and original.existed and original.backup then pcall(fs.copy, original.backup, path) end
    end
    term.setTextColor(colors.white)
    print("Rollback completed. Backup folder: " .. backupDir)
    return
end

local marker = fs.open("/mcos/data/installed_version.txt", "w")
if marker then marker.write(VERSION) marker.close() end

print("")
term.setTextColor(colors.lime)
print("McOS 1.0 installed successfully.")
term.setTextColor(colors.white)
print("Existing settings, user files and external apps were preserved.")
print("Pre-update copies: " .. backupDir)
print("Run: reboot")
